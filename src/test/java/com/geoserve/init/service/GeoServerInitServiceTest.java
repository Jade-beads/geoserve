...
class GeoServerInitServiceTest {
    ...
    void initializeRunsSingleWorkspaceDatastoreAndSixLayersInOrder() {
        ...
        when(client.ensureStyle(properties.getStyles().get(2)))
                .thenReturn(ResourceAction.created("style", "site_selection:score_style", "created"));
        ...
        // 顺序与生产链路一致：workspace -> 3 styles -> datastore -> 6 layers -> 1 GWC。
        assertThat(result.getActions()).hasSize(12);
        ...
        inOrder.verify(client).ensureStyle(properties.getStyles().get(2));
    ...
    private GeoServerInitProperties properties() {
        ...
        Style scoreStyle = new Style();
        scoreStyle.setName("score_style");
        scoreStyle.setSldLocation("classpath:styles/score-style.sld");
        ...
        properties.setStyles(Arrays.asList(countStyle, priceStyle, scoreStyle));
