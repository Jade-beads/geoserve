...
- 为图层设置默认样式。
- 只为 `basic_all` 创建 GeoWebCache WMTS 配置，使用 `WebMercatorQuad` 和 `image/png`。
...
GeoServer 有 JMS Cluster 方案，但它主要用于配置同步，不负责业务数据和切片数据分发。当前项目不启用 JMS 集群，按两套独立 GeoServer 部署处理。GeoWebCache 切片缓存通过 `GEOWEBCACHE_CACHE_DIR` 独立配置，可指向 A/B 都能访问的共享盘目录；托管启动时会把该目录写入 GeoServer `webapps/geoserver/WEB-INF/web.xml`。严格生产多实例缓存方案仍建议评估 standalone GeoWebCache。
...
该目录保留了 `.gitkeep`，真实 ZIP 包和 JAR 包已在 `.gitignore` 中忽略，不会提交到 GitHub。当前实现只解压 ZIP 包；压缩包内需要包含 `bin/startup.sh`、`bin/shutdown.sh`、`webapps/geoserver/WEB-INF/lib` 和默认 `data_dir/security/usergroup/default/users.xml`。

如果要求任何时候都不能使用默认 `admin/geoserver` 登录，需要先在本地 GeoServer 中把 admin 密码改成目标密码，然后取出 `data_dir/security/usergroup/default/users.xml` 里对应用户的 `password` 密文，配置到：

```bash
export GEOSERVER_USERNAME=admin
export GEOSERVER_PASSWORD='你的明文密码'
export GEOSERVER_DEPLOY_ADMIN_PASSWORD_ENCODED='digest1:你的本地密文'
```

`GEOSERVER_PASSWORD` 只用于初始化服务后续 REST 登录；`GEOSERVER_DEPLOY_ADMIN_PASSWORD_ENCODED` 会在 GeoServer 启动前替换 active `GEOSERVER_DATA_DIR/security/usergroup/default/users.xml` 中的 admin 密码密文，不会打印到日志。不要配置 `plain:` 明文编码。
...
4. 如果配置了 `GEOSERVER_DEPLOY_ADMIN_PASSWORD_ENCODED`，启动前替换 active `data-dir` 里的 `security/usergroup/default/users.xml`；如果 active `data-dir` 没有该文件，会先从解压包自带 `data_dir` 拷贝缺失文件，再替换密文。
5. 替换 GeoServer JDBC 驱动：删除 `postgresql*.jar`，复制 `gsjdbc4.jar`。
6. 启动前更新 GeoServer `webapps/geoserver/WEB-INF/web.xml`，写入实际 GWC 切片目录 `GEOWEBCACHE_CACHE_DIR`。
7. 设置 `GEOSERVER_DATA_DIR`、`GEOWEBCACHE_CACHE_DIR`、`GEOSERVER_LOG_LOCATION`、`JAVA_HOME`、`JAVA_OPTS`。
8. 执行 GeoServer `bin/startup.sh`。
9. 轮询 `GET /rest/about/version`，等待 GeoServer 可用。
10. 如果 `GEOSERVER_INIT_RUN_ON_STARTUP=true`，再执行 workspace、style、datastore、layer、GWC 初始化。
...
默认开启 `GEOSERVER_DEPLOY_CACHE_DIR_PER_HOST_ENABLED=true`，代码会按本机 IP 自动派生实际 GWC 切片目录，并在 GeoServer 启动前写入 `webapps/geoserver/WEB-INF/web.xml` 的 `GEOWEBCACHE_CACHE_DIR`，同时注入环境变量 `GEOWEBCACHE_CACHE_DIR` 和 `JAVA_OPTS -DGEOWEBCACHE_CACHE_DIR`。例如某个节点按本机 IP 派生后的实际 GWC 切片目录为：
...
| `GEOSERVER_DEPLOY_ADMIN_USER_NAME` | 启动前替换密码密文的 GeoServer 管理员用户名 | `admin` |
| `GEOSERVER_DEPLOY_ADMIN_PASSWORD_ENCODED` | 写入 `users.xml` 的 GeoServer 密码密文；为空时不改写 | 空 |
| `GEOSERVER_DEPLOY_USERS_XML_PATH` | 相对 `GEOSERVER_DATA_DIR` 的用户文件路径 | `security/usergroup/default/users.xml` |
